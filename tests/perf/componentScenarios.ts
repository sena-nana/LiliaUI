...
import { defineComponent, h, type Plugin, type Ref, type VNode } from "vue";
...
export const componentPerformanceScenarios: ComponentPerfScenario[] = [
  ...
  },
  {
    ...
    render: (step) => h(Dropdown, {
      ...
      options: [
        { value: "compact", label: "Compact", hint: "Default" },
        { value: "dense", label: "Dense", hint: "High signal" },
    ...
    interact: (root) => click(root, "[data-agent-id='perf.dropdown']"),
  },
  {
    name: "LiliaAppRoot",
  ...
  {
    name: "UiIconButton",
    render: (step) => h(UiIconButton, { icon: Settings, label: "设置", active: step.value % 2 === 1 }),
    interact: (root) => click(root, ".ui-icon-button"),
  },
  {
    name: "UiInput",
