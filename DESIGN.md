...
- `packages/ui/src/styles/workspace.css` 维护 `LiliaWorkspace` + Region 布局与折叠动画。
- `packages/ui/src/styles/sidebar.css` 维护 `.secondary-panel` 侧栏 frame 基础样式,供 `LiliaSidebarFrame` 消费。
