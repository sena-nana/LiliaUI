...
- 旧 Router-owning `LiliaDesktopShell` 已移除并复活为 Router-free 组合 Shell(`@lilia/ui/shell` 的 `LiliaDesktopShell`、`@lilia/nana-ui/shell` 的 `NanaDesktopShell`);两套 Layer 的 AppShell 都不得依赖固定侧栏、RouterView 或 route meta 信息架构。
