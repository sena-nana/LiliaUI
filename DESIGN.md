...
- `LiliaDesktopShell` 只作为阶段性兼容入口,新布局不得继续依赖其固定主侧栏或 route meta 信息架构。
...
- 常规公共 UI 变更至少运行 `pnpm perf:components:light`;涉及 overlay、菜单、搜索、壳层布局、Teleport、transition、滚动、resize 或全局监听时,同时运行 `pnpm perf:components:browser`。
- 性能 baseline 只能通过 `pnpm perf:components:update-baseline` 显式更新;更新前要确认数值变化来自有意的功能或结构变化,而不是本机噪声。
