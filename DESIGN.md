...
- `@lilia/ui-contract`: UI Layer 共享的组件、Policy 与 preset 类型契约。
- `@lilia/ui-foundation`: 无具体视觉的 primitive、共享状态机、commands 和 settings。
- `@lilia/ui`: 面向专业工具的 Professional Layer 视觉与交互实现（本仓库唯一官方视觉 Layer；业务样式分叉由消费应用自行实现）。
...
主题令牌由唯一视觉事实源 `@lilia/theme` 的 `packages/theme/src/base.css` 入口导入的 `packages/theme/src/styles/tokens.css` 维护（`@lilia/ui` 仅转发，不留副本）。新增颜色前先判断是否已有语义令牌可以表达。
...
- `@lilia/theme` 是唯一视觉基座事实源；`@lilia/ui` 的 `styles.css` 与 `styles/*.css` 只是 `@import "@lilia/theme/..."` 的转发文件，不复制内容。
- `packages/theme/src/base.css` 维护主题令牌入口、reset、字体契约、selection、基础表单控件和通用状态类（内部 `@import` tokens 与 state-layer）。
- `packages/theme/src/styles/workspace.css` 维护 `LiliaWorkspace` + Region 布局与折叠动画。
- `packages/theme/src/styles/sidebar.css` 维护 `.secondary-panel` 侧栏 frame 基础样式,供 `LiliaSidebarFrame` 消费。
- `packages/theme/src/styles/page.css` 维护页面基础结构,例如 `.page-header`、`.card`、`.kv`。
- 单组件 CSS 由组件自身导入,保留在 `@lilia/ui`;需要公开消费时通过 `@lilia/ui` 的细粒度 CSS export 暴露。
- 组件私有样式保留在对应 SFC 的 scoped style 中。
- 跨页面复用才进入 page 样式;Workspace Region 布局专属才进入 workspace 样式;单组件行为留在组件内或组件同目录 CSS。
...
- 列表行、导航行与侧栏行只显示单行主标签;过长文本省略,禁止在同一行控件内堆叠主标签 + 说明副行。设置行的 `label + hint` 不在此限。
...
- 旧 Router-owning `LiliaDesktopShell` 已移除并复活为 Router-free 组合 Shell(`@lilia/ui/shell` 的 `LiliaDesktopShell`);AppShell 不得依赖固定侧栏、RouterView 或 route meta 信息架构。
...
- 页面标题只保留紧凑 `h1`/`h2`;禁止在 `.page-header` 下添加说明性副标题或描述段落。状态放正文、状态区或行内元数据。页面头部只做定位,不要变成 hero。浮层 `description` 不受此限。
...
- 文本不会溢出紧凑容器。
- 列表、导航与侧栏行保持单行主标签,没有主标签 + 说明副行堆叠。
- `.page-header` 下没有说明性副标题或描述段落。
