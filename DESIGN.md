...
- 标题栏高度、原生窗口控件留白、侧栏宽度、resize 行为和设置页结构由 `@lilia/ui` 统一维护。
...
- 普通卡片使用 16px squircle/真实圆角基准,即 `--app-corner-radius: 16px` 与 `--radius-md`;该令牌只控制界面组件,不控制系统窗口外轮廓,也不要把默认卡片误改为 8px。
