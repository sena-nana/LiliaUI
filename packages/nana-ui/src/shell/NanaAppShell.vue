...
const props = withDefaults(defineProps<{
  ...
  contextTitle?: string;
  agentId?: string;
}>(), { sidebarMode: "expanded", contextVisible: false, contextTitle: "属性" });
const emit = defineEmits<{ "update:sidebarMode": [mode: SidebarMode]; selectNavigation: [item: SidebarItem] }>();
...
<template>
  <div class="nana-shell" :class="{ 'is-narrow': narrow }" :data-agent-id="agentId ?? 'nana.shell'">
    ...
    <NanaSidebar :items="navigation" :settings-item="settingsItem" :mode="effectiveMode" @update:mode="updateMode" @select="emit('selectNavigation', $event)"><template #header><slot name="sidebar-header" /></template><template #footer><slot name="sidebar-footer" /></template></NanaSidebar>
