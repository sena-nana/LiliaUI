...
`@lilia/ui-contract`、`@lilia/ui-foundation`、`@lilia/ui` 与 `@lilia/nana-ui` 使用独立包版本，但共享兼容队列：

- 仅 Layer 内部功能变化时，该 Layer 可以独立发布。
- Contract/Foundation 变化时，受影响的两个 Layer 必须在同一发布批次验证并声明兼容范围。
- major/minor/patch 依据各包公共 API 决定，不要求四个包永远使用相同版本号。
...
- `Nana Layer`：Consumer 组件、Policy、patterns、feedback、expressive。
- `Tooling and migration`：preset、migrate、config、build、模板检查。
...
```json
{
  "dependencies": {
    ...
    "@lilia/nana-ui": "github:sena-nana/LiliaUI#workspace=@lilia/nana-ui&commit=<sha>",
    "@lilia/tools": "github:sena-nana/LiliaUI#workspace=@lilia/tools&commit=<sha>"
  }
}
```

- Lilia preset 将 `@lilia/nana-ui` 换为 `@lilia/ui`，其余规则相同。
...
- local `portal:`/link 只改变依赖来源，不改变 lilia/nana preset；切回 remote 后必须恢复同 commit 锁定。

`ui-preset status` 会报告 Layer、facade、config、来源与 revision 漂移；发布前不得忽略 `workspace-revision` 或 `dependency-source` 错误。
...
2. 两个 Layer 的 Contract 调用、包导出与依赖方向通过；
3. 受影响组件的无障碍、视觉与性能证据更新；
4. Nana base-only bundle 不引入 Shell/pattern/expressive，lazy 入口保持异步 chunk；
5. Git workspace 使用同 commit 的消费安装验证通过；
6. Changelog 与迁移文档已覆盖 breaking/deprecated 项。
