...
`@lilia/ui-contract`、`@lilia/ui-foundation`、`@lilia/theme` 与 `@lilia/ui` 使用独立包版本，但共享兼容队列：
...
- `Theme`：唯一视觉事实源 `@lilia/theme` 的 token、state layer、reset、字体契约、基础布局 CSS 与 `applyTheme`。
- `Lilia Layer`：Professional 组件、Shell、诊断（视觉基座已下沉到 `@lilia/theme`）。
...
```json
{
  "dependencies": {
    ...
    "@lilia/theme": "github:sena-nana/LiliaUI#workspace=@lilia/theme&commit=<sha>",
...
- 同一应用使用的 `@lilia/*` Git workspace（包括 config/build/tools/theme）统一指向该批次的同一 commit，至少不允许 Layer、Contract、Foundation 与 Theme 分裂。
