...
| 根入口 `CalendarHeatmap` / `buildCalendarHeatmapModel` 等 calendar 导出 | 已移除的根入口重导出 | `@lilia/ui/calendar` | 已结束 | 当前 breaking release 已移除 |
| 根入口 `Dropdown` | 已移除的根入口重导出 | `@lilia/ui/search` | 已结束 | 当前 breaking release 已移除 |
| 根入口 `AnchoredActionMenu` / `ActionMenuItem` | 已移除的根入口重导出 | `@lilia/ui/overlay` | 已结束 | 当前 breaking release 已移除 |
| `@lilia/ui/settings` 的 `LiliaAppearanceSection` / `LiliaAboutSection` | 已移除的 settings barrel 连带导出 | `@lilia/ui/settings/sections` | 已结束 | 当前 breaking release 已移除 |
| `@lilia/ui/composables` 聚合 barrel（新代码） | 不推荐的聚合导入 | `@lilia/ui/composables/<name>` 深路径 | `0.2.x` | 最早 `0.3.0`,逐项 breaking changelog |
