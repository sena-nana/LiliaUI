...
桌面工作区从固定 Shell 侧栏迁移到统一 Region 的步骤见 [Workspace Region 布局与迁移](../workspace-regions.md)。旧 Router-owning Shell 已移除；Lilia 应用可直接使用组合好的 `LiliaDesktopShell`，或用 `LiliaAppShell` 与 Workspace Regions 自行组合。

官方仅提供 `@lilia/ui`：`ui.preset` 仅接受 `"lilia"`，并拒绝 `layout`。其他视觉由业务应用自行样式分叉，见 [架构边界](./architecture.md)。
...
| 维度 | 值 | 负责内容 |
| --- | --- | --- |
| preset | `lilia` | Layer 依赖、facade、样式、preset adapter、density/config |
| source | `local` / `remote` | workspace 包来自 portal/link 还是同 commit Git 依赖 |

preset 与来源的切换由消费应用自行管理；本仓库不再提供自动切换/迁移工具。
...
消费应用从旧 Router-owning Shell 或已移除的 NanaUI 切换时，以下评审分类仍由产品与业务团队负责：
