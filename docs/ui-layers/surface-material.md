...
List、Tree、Sidebar、Menu 使用起始边 indicator；Tabs、SegmentedControl、Toolbar Toggle 使用底边 indicator；Card 使用边缘 indicator。状态背景颜色只来自共享 State Layer，indicator 几何可以由组件类别声明。raised 只改变 Surface level，不创建另一套交互颜色。
